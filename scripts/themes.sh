...
cat "$repo_path/themes/mocha/$flavor.yml" | sed 's/^/  /' | head -n -3 >> $config_file

# yazi
config_dir=~/.config/yazi
curl -o $config_dir/theme.toml "https://raw.githubusercontent.com/catppuccin/yazi/refs/heads/main/themes/mocha/catppuccin-mocha-$flavor.toml"
